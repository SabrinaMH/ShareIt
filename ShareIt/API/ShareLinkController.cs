...
namespace ShareIt.API
{
    ...
    public class ShareLinkController : ApiController
    {
        ...
        public HttpResponseMessage Post([FromBody] ShareLinkInputModel model)
        {
            ...
            var emailOfSender = new EmailAddress(model.EmailOfSender);
            var nameOfSender = new Name(model.NameOfSender);
            var shareLink = new ShareLink(emailOfSender, nameOfSender, model.Subject, link, new ListOfReceivers(receivers.ToArray()));
            _bus.Send(shareLink);
